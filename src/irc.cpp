...
// Copyright (c) 2011-2012 coinempz Developers
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #coinempzTEST0\r");
            Send(hSocket, "WHO #coinempzTEST0\r");
        } else {
            // randomly join #coinempz00-#coinempz99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #coinempz%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #coinempz%02d\r", channel_number).c_str());
        }
