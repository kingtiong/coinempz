...
uint256 hashGenesisBlock("0xea2b51e7c8a4a2d893eaa63c8b9a3f231f3a9a6969b99f30b33ca3cd4482d826");
...
const string strMessageMagic = "coinempz Signed Message:\n";
...
{
    // coinempz: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    // coinempz: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "coinempz genesis";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
	    ...
	    assert(block.hashMerkleRoot == uint256("0xc688ac74c2a7f2d6c2cfa6af6b7e735cc5fd9273d109a051a566cbef3d1eda9c"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xc2, 0xce, 0xa3, 0xeb };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("coinempzMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("coinempzMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static coinempzMiner(CWallet *pwallet)
{
    printf("coinempzMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("coinempz-miner");
    ...
    try { loop {
        ...
        printf("Running coinempzMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("coinempzMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&coinempzMiner, pwallet));
}
